...
#'
#' @param filename file to process (Carstens AG501 .txt file, with header); assumes they have column names set by \code{\link{process_ag501}} and that the sampling rate of the EMA data is 250 Hz. Can read from .txt file (default) or from in-memory object (see `read_from_file`).
...
#' @param output_directory directory to save output SSFF files for each sensor
#'
#' @author Sam Kirkham
#'
#' @examples
...
#' ag501_to_ssff(filename = "sf2_0005.txt", sensor_array = sensors, pal_filepath = "sf2_palate.txt", pal_sensor = 15, output_directory = "path/to/dir")
#' }
#'
...
#' sensors <- c("TT", "TB", "TD", "LT", "UT", "UL", "LL")
#' lapply(files, ag501_to_ssff, sensor_array = sensors, pal_filepath = "sf2_palate.txt", pal_sensor = 15, output_directory = "path/to/dir")
#' }
#'
...
ag501_to_ssff <- function(filename, sensor_array, pal_filepath, pal_sensor, output_directory){
  ...
  for(i in 1:length(sensor_array)){
    ...
    wrassp::write.AsspDataObj(dobj = ado, file = paste0(output_directory, "/", output_name))
    }
  pal <- get_palate_trace(pal_filepath, pal_sensor)
  pal_name <-  paste0(gsub(".txt", ".", basename(filename)), "PAL")
  wrassp::write.AsspDataObj(dobj = pal, file = paste0(output_directory, "/", pal_name))
}
